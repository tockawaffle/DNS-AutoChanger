...
import { proxmoxJobDiscord } from "./proxmox";
...
export async function cfJob(client: Client) {
    const cfJob = new CronJob("* * * * *", async () => {
        const ip = await getPublicIp();
        const { id, name, type, content, proxied, ttl, zone_id } =
            await getIpDns(process.env.DNS_NAME as string);
        const channel = client.guilds.cache
            .get(process.env.SERVER_ID as string)
            ?.channels.cache.get(
                process.env.LOG_CHANNEL_ID as string
            ) as GuildChannel as TextChannel;
        if (ip !== content) {
            try {
                    ...
                    await channel.send({
                        embeds: [
                            new EmbedBuilder({
                                ...
                                .setColor("Red")
                                .setImage(
                                    "https://preview.redd.it/qlbz5kaucva51.jpg?auto=webp&s=36752e06b38e90bfd94ca0186163dbd338a4d7e5"
                                ),
                        ],
                    });
                } else {
...
export async function startJobOnInit(client: Client) {
    ...
    if (job!.proxmox_updater === true) {
        const pJob = await proxmoxJobDiscord(client);
        pJob.start();
